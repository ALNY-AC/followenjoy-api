    ...
    public function 邀请人得钱奖($sub){
        ...
        if($this->level=='1'){
            if(count($subList)>=25){
                $User=D('User');
                $data=[];
                $data['user_vip_level']=2;
                $where['user_id']=$this->getUserId();
                $User->where($where)->save($data);
            }
        }
        if($this->level=='2'){
            if(count($subList)>=750){
                $User=D('User');
                $data=[];
                $data['user_vip_level']=3;
                $where['user_id']=$this->getUserId();
                $User->where($where)->save($data);
            }
        }
    ...
    */
    ...
    public function 自己邀请直属会员得钱(){
        ...
        if($this->level==1){
            ...
            // 经理收益7：【团队新增会员获得125元】
            // 这一层是会员，邀请的下面的人，上级如果是经理，得到125
            if($this->getSuper()){
                // 有上级
                // 上级如果是经理，经理得到钱
                if($this->getSuper()->getLevel()==2){
                    $经理=$this->getSuper();
                    $经理->money+=125;
                    $经理->saveMoney();
                    // 如果还有上级，并且上级是经理，经理得到经理收益8
                    if($经理->getSuper()){
                        
                        if($经理->getSuper()->getLevel()==2){
                            // 上级是经理，上级经理得到20
                            $经理2=$经理->getSuper();
                            $经理2->money+=20;
                            $经理2->saveMoney();
                ...
                if($this->getSuper()->getLevel()==3){
                    // 上级不是经理，看看是不是总监，c被邀请，b是会员，a是总监
                    // 收益【7】
                    $总监=$this->getSuper();
                    $总监->money+=175;
                    $总监->saveMoney();
                    if($总监->getSuper()){
                        $总监->getSuper()->money+=80*0.3;
                        $总监->getSuper()->saveMoney();
                    }
                }
